...
//This production only for UTF-8
class IndentConverterU8 {
   static final Charset charCode = Charset.forName("UTF-8");
   static int TABS = 3;
   static int TABSM = 3;
...
//       if (!inputdir.getName().toLowerCase().endsWith(".java")) {
         ...
         errexit("Input is not file or directory.");
      }
...
//                System.err.println("The input file: " + input.getAbsolutePath() + " is not readable.");
               ...
               try {
                  ...
                  if (!outpath.exists()) {
                     outpath.mkdir();
                     if (!outpath.exists()) {
                        File inpath = input.getParentFile();
                        while(!outpath.exists()) {
                           inpath = inpath.getParentFile();
                           if(new File (inpath.getAbsolutePath().replace(inputpath, outputpath)).mkdir()) {
                              inpath = input;
                           }
                        }
                     }
...
//    if (str == null) {finish();}
   ...
   private static void convert(File inputfile, File outputfile) throws IOException{
      try(
         BufferedReader input = Files.newBufferedReader(inputfile.toPath(), charCode);
         BufferedWriter output = Files.newBufferedWriter(outputfile.toPath(), charCode);
      ){
         String line = null;
...
//             buffer.delete(0, TABS);
            ...
            if (input.charAt(i + 1) == '/') {   //comment sequence
               ...
               if (ismulticomment || i + 2 >= input.length()) {
                  break;
               }
               ismulticomment = true;
               i++;
